...
class PointCloudObjectDetection : public rclcpp::Node {
 ...
 private:
  ...
  std::atomic<bool> manifest_params_update_pending_{false};
  std::atomic<bool> manifest_params_update_in_progress_{false};
  std::mutex manifest_params_mutex_;
  std::vector<rclcpp::Parameter> pending_manifest_params_;
};
